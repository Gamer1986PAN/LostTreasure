...
function LT:SlotAdded(bagId, slotIndex, slotData)
  ...
  local isTreasureMapText = zo_plainstrfind(zo_strlower(slotData.name), TREASURE_TEXT[lang])
  local isSurveyMapText = zo_plainstrfind(zo_strlower(slotData.name), SURVEYS_TEXT[lang])

  local itemID = select(4, ZO_LinkHandler_ParseLink(GetItemLink(BAG_BACKPACK, slotIndex)))
  slotData.itemID = tonumber(itemID)

  if (isTreasureMapText or slotData.specializedItemType == SPECIALIZED_ITEMTYPE_TROPHY_TREASURE_MAP) and LT.SavedVariables["treasureMarkMapMenuOption"] == 2 then
    LT:refreshTreasurePins()
  end
  if (isSurveyMapText or slotData.specializedItemType == SPECIALIZED_ITEMTYPE_TROPHY_SURVEY_REPORT) and LT.SavedVariables["surveysMarkMapMenuOption"] == 2 then
